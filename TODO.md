...
### Smart Resume — Open Loops (Apr 15, 2026)
Smart Resume shipped overnight Apr 15 (`ss_offset` snapshot + high-water-mark save + 92% completion reset + duration-aware reaper grace + SRT shifter). Hard-won lessons pinned in [CLAUDE.md](CLAUDE.md). These are the rough edges still standing:

- [ ] **Explicit `--seek N` should reset the saved high-water mark**, not be high-water'd against it. Tonight's UX failure: user ran `spela play 3 --no-intro` expecting a fresh-from-start restart, but `save_position_smart`'s HWM logic preserved the stored 2236s from prior playback, so the new `do_play`'s auto-resume picked up the old position. Workaround was `spela stop` → `spela clear tt1190634` → `spela play 3 --seek 0 --no-intro`. Fix: `do_play` should treat `--seek N` (including `--seek 0`) as an explicit user action that bypasses auto-resume AND overwrites the stored resume position for that IMDb ID / title slug, because the user's intent is "I want the playhead HERE now", not "here, unless I was further along before". Principle: explicit user actions override remembered state.

- [ ] **Sanity-check `cast_health_monitor` position saves against wall-clock delta.** During the S05E02 restart debug, two consecutive 30s poll cycles reported `absolute` positions 2236s → 2454s (+1796s advance) in ~60s of wall time, which is physically impossible for normal playback. Most-likely root cause: the Chromecast reported stale `current_time` from a pre-deploy stream that outlived the old spela process, combined with the new +440s `ss_offset` from auto-resume, producing a phantom 37-min jump that poisoned the HWM. Defense: in `cast_health_monitor`, track `last_poll_wall` + `last_absolute`; if a new poll shows `delta_absolute > 2.0 * delta_wall + 60s`, log a warning and SKIP the `save_position_smart` call for that tick. Catches the "Chromecast remembered a stream that survived our restart" class without blocking legitimate fast-forward (user seeks update wall-time baseline via `/api/seek-restart`).

- [ ] **Visible auto-resume notification.** When `do_play` restores a saved position, nothing tells the user it happened — they just see the episode start N minutes in and wonder if the code is broken. Options: (1) brief Ruby TTS "resuming at 37 minutes" before cast (mirrors the engine-side confirm-before-streaming pattern in `conversation_engine.py::_execute_spela`), (2) spela CLI prints `↩ Resuming at 0:37:20 (from saved position)` to stdout, (3) Custom Cast Receiver overlay with fade-out. Start with (2) because it's a 3-line change in `do_play`'s response struct and needs no new UI.

- [ ] **Real release-name corpus test for the ranker.** The `is_hevc_from_title` space-separated bug slipped through 58 unit tests because none of them used real scene-release strings. Add `tests/ranker_corpus.rs` that loads a fixture of ~50 real Torrentio titles (anonymized: replace specific show names with generic placeholders but keep codec/resolution/tag layout intact) and asserts ordering matches expectation. Includes the Apr 15 FLUX vs playWEB diagnostic case as a regression pin. Generic pattern: parsers for external format (torrent titles, user agents, SRT timestamps) need corpus-based tests, not just unit tests of their own functions.

- [ ] **Custom Cast Receiver — Google Cast SDK $5 registration.** User offered to pay overnight; not yet done. Registration unlocks the Custom Receiver App ID needed by `static/cast-receiver.html` (Shaka Player + CAF v3). Benefits: (1) **native seeking** — Shaka does HLS seeking by segment index, the current server-side `/api/seek-restart` restart-the-transcode-from-new-ss approach becomes unnecessary; (2) **text-track handling** — can send WebVTT subtitles separately via Cast protocol instead of burning into video, eliminating the subtitle shift dance in `transcode.rs::shift_srt`; (3) **loading state UI** — replaces the blue-cast-icon "Black Flash" between clips. Blocking step: register at `cast.google.com/publish`, pay $5, get Receiver App ID, set it in `~/.config/spela/config.toml`, switch `cast.rs::cast_url` to load the custom receiver instead of Default Media Receiver. Spec lives at `.claude/specs/custom-cast-receiver/`.
