...
- [ ] **spela search returns spurious non-matching shows** — searching "The Boys" S05E03 returns `The.French.Chef.Season.05.03of20.Queen.of.Sheba.Cake` as a candidate (820 seeds!). Torrentio should only return results for the queried IMDb ID. Diagnose: is spela hitting the IMDb-ID endpoint or falling back to a name search? Add a post-filter that drops results whose title doesn't contain the requested show's canonical name tokens (token-based, tolerant to separators). Or at minimum mark non-matching results so Ruby can skip them.
