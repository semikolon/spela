...
- **Chromecast Default Media Receiver rejects spela's chunked-transfer fMP4** (Apr 15) — `cast_health_monitor` confirmed live: `player_state=IDLE` for The.Boys.S05E01 even with healthy ffmpeg + perfect Local Bypass + cast_url returning OK. Root cause is spela's `/stream/transcode` endpoint: serves a growing fragmented MP4 with `Transfer-Encoding: chunked`, no `Content-Length`, and always returns `200 OK` (never `206 Partial Content`). Default Media Receiver's MP4 parser doesn't accept that combo and drops to IDLE without any error surface. Confirmed against [Igalia/cog#463](https://github.com/Igalia/cog/issues/463): "media framework requires either a known content length or complete data delivery, not incremental chunked delivery". **Workaround that proves the diagnosis**: the same FLUX file remuxed via `ffmpeg -c:v copy -c:a aac -movflags +faststart` and served by `catt` (which uses 206 Range responses with Content-Length) plays perfectly — same content, same Chromecast, same network, just the right serving strategy. **The proper fix is the HLS rework tracked in `TODO.md` § "HLS rework for /stream/transcode"**: rewrite the endpoint to output ffmpeg HLS segments + manifest with proper byte ranges, which Default Media Receiver supports natively
