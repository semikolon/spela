...
- `static/remote.html` — web-remote SPA, **Phases 1-4 LIVE** (v3.7): single no-build dark asset, `include_str!`'d, served at `/remote` (bare `/`→307). Hash-routed shell + skeleton→titled-fallback poster grid + abortable API client + debounced search/Movies-TV toggle + hybrid play ("more sources") + target picker ("This phone" = disabled Phase-6 placeholder) + now-playing (`/status`+`/api/position` interpolated) + scrubber/⏮ (frontier derived from `/hls/playlist.m3u8`). Phase 5 (My-Library view) + Phase 6 (phone-direct) pending
