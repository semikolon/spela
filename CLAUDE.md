...
- `static/remote.html` — web-remote SPA, **Phases 1-5 LIVE** (v3.7): single no-build dark asset, `include_str!`'d, served at `/remote` (bare `/`→307). Hash-routed shell + skeleton→titled-fallback poster grid + abortable API client + debounced search/Movies-TV toggle + hybrid play ("more sources") + target picker ("This phone" = disabled Phase-6 placeholder) + now-playing (`/status`+`/api/position` interpolated; play-state is **intent-only**, never reconciled from polling) + scrubber/⏮ (frontier from `/hls/playlist.m3u8`) + My-Library view (`GET /library`; unreachable/offline/empty/grid distinct per AC-3.4; tap → `/play` by `raw_name` via the bridge). Phase 6 (phone-direct) pending
