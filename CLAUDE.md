...
- **Chromecast hardcodes Google DNS, can NOT resolve LAN hostnames** (Apr 15) — Chromecast devices use 8.8.8.8 / 8.8.4.4 by default and ignore the LAN's recursive resolver (AdGuard Home, dnsmasq, mDNS rewrites). spela's cast LOAD URL is built from `config.stream_host`, so a hostname like `darwin.home` makes the receiver fetch a name it can't resolve → LOAD silently fails → `player_state=IDLE` forever, while every other LAN client (Mac Mini, MERIAN, etc.) reaches spela through the LAN DNS and sees no problem. **`stream_host` MUST be a private LAN IP** (e.g. `192.168.4.1`) for chromecast targets to work. spela startup now WARNs if `stream_host` looks like a hostname. The Apr 15 IP-vs-hostname diagnostic was: pychromecast LOAD with `http://darwin.home:7890/hls/master.m3u8` → no manifest fetch in spela's access log; LOAD with `http://192.168.4.1:7890/hls/master.m3u8` → multiple manifest hits + segment hits + `state=PLAYING` within 2 seconds
